	...
	struct device_attribute *attr, char *buf)
{
	...
	if (!(cm3663->power_state & PROXIMITY_ENABLED)) {
		...
		regulator_enable(cm3663->regulator);
	...
	if (!(cm3663->power_state & PROXIMITY_ENABLED)) {
		...
		regulator_disable(cm3663->regulator);
		mutex_unlock(&cm3663->power_lock);
	}
			  ...
			  const struct i2c_device_id *id)
{
	...
	cm3663->regulator = regulator_get(&client->dev, "vled");
	if (IS_ERR(cm3663->regulator)) {
...
err_setup_reg:
	...
	regulator_put(cm3663->regulator);
err_get_vreg:
	kfree(cm3663);
done:
	return ret;
