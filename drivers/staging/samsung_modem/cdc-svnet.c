...
//TODO:
// 1. Platform device with pdata for power control/irq
// 2. Suspend/resume wait events/state tracking
// 3. cleanup/remove static variables
...
int usbsvn_request_resume(void)
{
	...
	if (share_svn->dpm_suspending) {
		...
		pr_debug("%s: suspending skip host wakeup\n",
			__func__);
		return 0;
	}
	usb_mark_last_busy(share_svn->usbdev);
	if (share_svn->resume_debug >= 1) {
		pr_debug("%s: resumeing, return\n", __func__);
		return 0;
	}

	//if (dev->power.status != DPM_OFF) {
		pr_debug("%s:run time resume\n", __func__);
...
/*	char *envs[2] = {"MAILBOX=hsic_disconnected", NULL };*/
	...
	if (svn->usbsvn_connected) {
		pr_info("svn re-connected\n");
		goto out;
	}
	if (svn->dpm_suspending) {
		pr_info("svn suspending, delay reconnection\n");
		goto retry;
	}
...
static int usbsvn_write(struct net_device *dev, struct sipc4_tx_data *tx_data)
{
	...
	if (err < 0) {
		pr_err("%s: usbsvn_initated_resume fail\n", __func__);
		goto exit;
	}
...
static int __init usbsvn_init(void)
{
	...
	err = _netdev_init();
	if (err < 0) {
		pr_err("%s: netdev register fail\n", __func__);
		goto fail_netdev;
	}

	err = usb_register(&usbsvn_driver);
	if (err < 0) {
		pr_err("%s: usb register fail\n", __func__);
		goto fail_usb;
	}
fail_usb:
	_netdev_exit();
